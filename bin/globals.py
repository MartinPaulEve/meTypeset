...
class GV (Debuggable):
    def __init__(self, settings, debug):
        ...
        if not settings.args['bibscan']:
            ...
            self.input_metadata_file_path = settings.args['--metadata'] if settings.args[
                '--metadata'] else os.path.join(settings.script_dir,
                                                settings.value_for_tag('default-metadata-file-path', self))
            ...
            self.docx_word_temp_folder_path = settings.clean_path(
                settings.concat_path(self.docx_temp_folder_path, settings.value_for_tag('word', self)))

            self.word_document_xml = settings.clean_path(
                self.docx_temp_folder_path + '/' + settings.value_for_tag('word-document-xml', self))

            self.docx_style_sheet_dir = settings.concat_path(self.script_dir,
                                                             settings.value_for_tag('docs-style-sheet-path',
                                                                                    self))
            self.proprietary_style_sheet = settings.concat_path(self.script_dir,
                                                                settings.value_for_tag('proprietary-math-stylesheet',
                                                                                       self))

            self.docx_to_tei_stylesheet = settings.clean_path(
                self.docx_temp_folder_path + '/' + settings.value_for_tag('doc-to-tei-stylesheet', self))

            self.docx_media_path = settings.clean_path(
                settings.concat_path(self.docx_word_temp_folder_path, settings.value_for_tag('media', self)))

            self.output_media_path = settings.clean_path(
                settings.concat_path(self.output_folder_path, settings.value_for_tag('outputmedia', self)))
            ...
            #TEI paths
            self.tei_folder_path = settings.clean_path(self.output_folder_path + '/' + settings.value_for_tag('tei',
                                                                                                              self))
            ...
            self.nlm_style_sheet_dir = settings.clean_path(settings.concat_path(settings.script_dir,
                                                                        settings.value_for_tag('tei-to-nlm-stylesheet',
                                                                                           self)))
            ...
            self.metadata_style_sheet_path = settings.clean_path(
                settings.concat_path(settings.script_dir, settings.value_for_tag('metadata-stylesheet',
                                                                                 self)))
        ...
        self.database_file_path = settings.clean_path(settings.concat_path(os.path.join(settings.script_dir, 'database'),
                                                                   settings.value_for_tag('databasefile', self)))
    ...
    def generate_path(self, settings, tag, path):
        return settings.clean_path(settings.concat_path(path, settings.value_for_tag(tag, self)))

    def mk_dir(self, path):
        self.debug.mkdir(path)

    def setting(self, tag):
        return self.settings.value_for_tag(tag, self)
    ...
    def set_java_classpath(self):
        ...
        for lib in self.settings.value_for_tag('saxon-libs', self).strip().split(";"):
